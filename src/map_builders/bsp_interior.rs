...
impl BspInteriorBuilder {
    ...
    fn build(&mut self) {
        ...
        self.add_subrect(first_room, &mut rng);
    ...
    fn add_subrect(&mut self, rect: Rect, rng: &mut RandomNumberGenerator) {
        ...
        if split <= 2 {
            ...
            self.rects.push(h1);
            if half_width > MIN_ROOM_SIZE { self.add_subrect(h1, rng); }

            let h2 = Rect::new(rect.x1 + half_width, rect.y1, half_width, height);
            self.rects.push(h2);
            if half_width > MIN_ROOM_SIZE { self.add_subrect(h2, rng); }
        } else {
            let v1 = Rect::new(rect.x1, rect.y1, width, half_height - 1);
            self.rects.push(v1);
            if half_height > MIN_ROOM_SIZE { self.add_subrect(v1, rng); }

            let v2 = Rect::new(rect.x1, rect.y1 + half_height, width, half_height);
            self.rects.push(v2);
            if half_height > MIN_ROOM_SIZE { self.add_subrect(v2, rng); }
        }
    }
